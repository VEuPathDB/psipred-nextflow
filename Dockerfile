...
Run cd /usr/local/share  &&  wget http://bioinfadmin.cs.ucl.ac.uk/downloads/psipred/old_versions/psipred.4.0.tar.gz  && tar -zxvf psipred.4.0.tar.gz  && cd psipred/src  && make  && make install  && rm /usr/local/share/psipred.4.0.tar.gz  && cd /usr/local/share  && wget ftp://ftp.ncbi.nih.gov/blast/executables/legacy.NOTSUPPORTED/2.2.26/blast-2.2.26-x64-linux.tar.gz  &&  tar -zxvf blast-2.2.26-x64-linux.tar.gz  && rm /usr/local/share/blast-2.2.26-x64-linux.tar.gz  &&  cd psipred  &&  rm runpsipred_single
